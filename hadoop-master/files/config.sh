...
for i in  $(seq $(serf members|grep slave|wc -l)) ; do echo slave$i.gongkun.com >> /usr/local/hadoop/etc/hadoop/slaves ; done;
...
cc=0;cat /etc/hosts|grep hadoop-slave|while read line ; do ((cc++)); echo $line slave$cc.gongkun.com >> /etc/hosts; done;
